...
class BimMobileWarehouse(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', "New") == "New":
            vals['name'] = self.env['ir.sequence'].next_by_code('bim.mobile.warehouse') or "New"
        result = super(BimMobileWarehouse, self).create(vals)
        result.create_warehouse()
        return result

    name = fields.Char('Code', default="New")
    reference = fields.Char('Description', required=True)
    ...
    picking_in_count = fields.Integer('Quant. Receptions', compute="_compute_picking")
    picking_out_count = fields.Integer('Quant. Deliveries', compute="_compute_picking")
    ...
    # ~ state = fields.Selection(
        # ~ [('New', 'New'),
         ...
         # ~ ('Cancelled', 'Cancelled')],
        # ~ 'Estado', default='New', tracking=True)
    # ~ product_ids = fields.One2many('product.list', 'requisition_id', string='Listado Products')
    # ~
