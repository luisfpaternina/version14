...
class BimWorkOrder(models.Model):
    ...
    project_id = fields.Many2one('bim.project', 'Project', copy=True, domain="[('company_id','=',company_id)]")
    ...
    date_order = fields.Datetime(string='Date Order', required=True, readonly=True, index=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, copy=False, default=fields.Datetime.now, help="Creation date of draft/sent orders,\nConfirmation date of confirmed orders.")
    validity_date = fields.Date(string='Validity Date', readonly=True, copy=False, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},
    ...
    user_id = fields.Many2one(
        'res.users', string='Vendor', index=True, tracking=2, default=lambda self: self.env.user,
    ...
    pricelist_id = fields.Many2one(
        'product.pricelist', string='Price List', check_company=True,  # Unrequired company
    ...
    order_line = fields.One2many('bim.work.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)

    invoice_count = fields.Integer(string='Invoice Count', compute='_get_invoiced', readonly=True)
    move_ids = fields.One2many('account.move', 'workorder_id', 'Assets')
    ...
    note = fields.Text('Conditions and Terms', default=_default_note)

    amount_untaxed = fields.Monetary(string='Tax Base', store=True, readonly=True, compute='_amount_all', tracking=5)
    ...
    payment_term_id = fields.Many2one(
        'account.payment.term', string='Payment Term', check_company=True,  # Unrequired company
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]",)
    ...
    commitment_date = fields.Datetime('Shipping Date',
                                      states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},
    ...
    amount_discount = fields.Float('Discount', compute='_compute_amount_undiscounted')
...
class BimWorkOrderLine(models.Model):
    ...
    product_uom = fields.Many2one('uom.uom', string='Unit', domain="[('category_id', '=', product_uom_category_id)]")
    ...
    qty_delivered = fields.Float('Quantity Sent', copy=False, compute='_compute_qty_delivered', inverse='_inverse_qty_delivered', compute_sudo=True, store=True, digits='Product Unit of Measure', default=0.0)
    qty_delivered_manual = fields.Float('Manually Sent', copy=False, digits='Product Unit of Measure', default=0.0)
    qty_to_invoice = fields.Float(
        compute='_get_to_invoice_qty', string='Quantity to Invoice', store=True, readonly=True,
        digits='Product Unit of Measure')
    qty_invoiced = fields.Float(
        compute='_get_invoice_qty', string='Quantity Invoiced', store=True, readonly=True,
        digits='Product Unit of Measure')
    ...
    analytic_tag_ids = fields.Many2many(
        'account.analytic.tag', string='Analytics Tags',
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
