...
class BimMaintenance(models.Model):
    ...
    _description = "BIM Maintenance Orders"
    _order = 'id desc'
    ...
    date_planned = fields.Datetime(string='Estimated Date', required=True,
        readonly=True, index=True, states={'draft': [('readonly', False)]},
        copy=False, default=fields.Datetime.now)
    date_done = fields.Datetime(string='Execution Date',
        readonly=True, index=True, states={'draft': [('readonly', False)]},
        copy=False, default=fields.Datetime.now)
    partner_id = fields.Many2one('res.partner', string='Customer', readonly=True,
        states={'draft': [('readonly', False)]}, required=True, change_default=True,
        index=True, tracking=True)
    project_id = fields.Many2one('bim.project', 'Project', readonly=True,
        required=True, copy=False, states={'draft': [('readonly', False)]})
    invoice_ids = fields.One2many('account.move', 'maintenance_id', 'Invoices')
    invoice_count = fields.Integer('# Invoices', compute=_compute_invoice)
    ...
    line_ids = fields.One2many('bim.maintenance.line', 'maintenance_id',
        string='Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)
    amount_total = fields.Monetary('Total', compute="_compute_total", store=True)
    requisition_ids = fields.One2many('bim.purchase.requisition','maintenance_id','Mat. Request')
    req_count = fields.Integer('Quantity Mat. Request', compute="_compute_req_count")
    maintenance_duration = fields.Integer('Estimated Duration (days)', default=1)
    department_id = fields.Many2one('bim.department', 'Departamnt', related="project_id.department_id", store=True)
    invoice_amount = fields.Monetary('Amount to Invoice')
    maintenance_currency_id = fields.Many2one('res.currency', 'Currency', related="project_id.maintenance_currency_id",
                                              store=True)
    reminder = fields.Boolean('reminder', compute='compute_reminder')
    days_reminder = fields.Integer('days reminder', compute='compute_days_reminder')
    ...
    def action_send(self):
        ...
        for mant in maintenances:
            if mant.reminder:
                ...
                if mail:
                    mant.message_post(
                        body=_("Email to Support: %s" % mant.project_id.customer_id.name))

    @api.model
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('bim.maintenance') or 'New'
    ...
    def generate_bim_req(self):
        ...
        if len(req_lines) == 0:
            raise UserError(_('There are no products to request'))
...
class BimMaintenanceLine(models.Model):
    ...
    price_subtotal = fields.Float("Balance", compute='_compute_subtotal')
    maintenance_id = fields.Many2one('bim.maintenance', string="Maintenance", ondelete='cascade')
...
class BimMaintenanceTagsDays(models.Model):
    ...
    _description = "Days remaining maintenance"

    name = fields.Integer('Days')