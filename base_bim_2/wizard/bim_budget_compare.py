...
class BimBudgetCompare(models.TransientModel):
    ...
    _description = 'Budgets comparator'
    ...
    origin_budget_id = fields.Many2one('bim.budget', 'Origin Budget', readonly=True)
    compare_budget_id = fields.Many2one('bim.budget', 'Budget to Compare', readonly=True)
    compare = fields.Selection([('chapter', 'Chapters'),
                                ('departure', 'Budget Items'),
                                ('both', 'All')], 'Compare', default='both', required=True)
    budget_type = fields.Selection([('budget', 'Budget'),
                                    ('cert', 'Certification')], 'Type', default='budget', required=True)
    price = fields.Boolean('Price', default=True)
    text = fields.Boolean('Text', default=True)
    quantity = fields.Boolean('Quantity', default=True)
    measure = fields.Boolean('Measurement line', default=True)
    ...
    def _onchange_quantity(self):
        if self.measure and not self.quantity:
            return {
                'warning': {
                    'message': '"Quantity" is mandatory if "Measurement line" is checked.'
                },
                'value': {
                    'quantity': True,
                }
    ...
    def print_excel(self):
        if not self.price and not self.text and not self.quantity and not self.measure:
            raise ValidationError(_('You must choose at least one option to compare out of price, text, quantity or measurement line.'))
        ...
        # Cabecera
        sheet.write(0, 2, _('Quant'), bold_left)
        sheet.write(0, 3, _('Price'), bold_left)
        if self.measure:
        ...
        sheet.write(0, 12, _('Quant'), bold_left)
        sheet.write(0, 13, _('Price'), bold_left)
        if self.measure:
        ...
        sheet.write(0, 22, _('Quant'), bold_left)
        sheet.write(0, 23, _('Price'), bold_left)
        if self.measure:
        ...
        for origin_concept in self.origin_budget_id.concept_ids:
            ...
            if self.measure:
                for i, line in enumerate(origin_concept.measuring_ids, 1):
                    ...
                    sheet.write(row + i, 9, round(line.amount_subtotal,2), normal_right)
        ...
        filename = '%s_%s_%s.xls' % ('_certification' if self.budget_type == 'cert' else '', self.origin_budget_id.code, self.compare_budget_id.code)
