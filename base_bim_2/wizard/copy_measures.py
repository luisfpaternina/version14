...
class CopyMeasuresWizard(models.TransientModel):
    ...
    _description = 'Wizard to copy measurements from budget items'

    concept_id = fields.Many2one('bim.concepts', 'Origin', required=True)
    dest_concept_id = fields.Many2one('bim.concepts', 'Destination', required=True)
