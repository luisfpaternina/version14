...
class BimGanttExport(models.TransientModel):
    ...
    _description = 'Gantt project exporter'
    ...
    def print_xml(self):
        if self.gantt_type == 'gp':
            raise ValidationError(_('Gantt Project is not implemented yet'))
        elif self.gantt_type == 'ms':
            xml = self.generate_ms_project()
        else:
            raise ValidationError(_('You must choose some type of gantt to generate.'))
    ...
    def generate_ms_project(self):
        ...
        for i, resource in enumerate(self.budget_id.concept_ids, 1):
            ...
            assignment_vals = {
                ...
                'ResourceUID': products.index(resource.product_id) if resource.product_id in products else 0,
