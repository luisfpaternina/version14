...
class BimRequisitionSpace(models.TransientModel):
    ...
    _description = 'Create Space Material Request'
    ...
    space_id = fields.Many2one('bim.budget.space', 'Space', default=default_get_space)
    type = fields.Selection([
        ('all', 'All Resources'),
        ('one', 'Per Resource Category')],
        'Type', default='all')
        ...
        for me in concept.measuring_ids:
            if me.space_id.id == self.space_id.id:
                if concept.quantity == 0:
                    raise UserError(_("Concept: {} has 0 quantity. Please specify some quantity to continue").format(concept.name))
    ...
    def create_requisition(self):
                ...
                if valid_product:
                    if product.id in product_ids:
                        ...
                        val = {
                            ...
                            'cost': resource.product_id.standard_price,
        ...
        if not products:
             raise ValidationError(_('There is not Resource to Process'))
