...
class BimIOStockMobile(models.TransientModel):
    ...
    _description = "Entry and Exit Mobile Warehouse"
    ...
    project_id = fields.Many2one('bim.project', string='Destination Location')
    warehouse_id = fields.Many2one('stock.warehouse', string='Location', related='mobile_id.warehouse_id')
    location_id = fields.Many2one('stock.location', string='Location Origin')

    type = fields.Selection([
        ('in', 'Entry Movement'),
        ('out','Exit Movement')],
        'Operation Type', default='in')
    ...
    def action_load(self):
        ...
        if self.type == 'in':
            ...
            if not self.project_id.stock_location_id:
                raise ValidationError(_('There is no Stock Location associated with the destination Work'))
...
class BimIOStockMobileLines(models.TransientModel):
    ...
    qty_available = fields.Float('Available',digits='BIM qty', compute="_get_qty_location")
